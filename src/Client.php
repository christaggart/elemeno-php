...
class Client
{
    ...
    private function get($path, $query = null)
    {
        ...
        try {
            $res = $this->httpClient->get($this->baseUrl . $path, $headers, $options)->send();
    ...
    {
        return $this->get($this->collectionBase . $collectionSlug . '/items',
            $this->getQuery($options, array('filters', 'sort')));
    }

    function getCollectionItem($collectionSlug, $itemSlug, $options = null)
    {
        return $this->get($this->collectionBase . $collectionSlug . '/items/' . $itemSlug,
            $this->getQuery($options, array('sort')));
    }
}
